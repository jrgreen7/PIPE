...
processed_data_path = "/home/williamm/scratch/final-wrap-up/PIPE/Deep-PIPE-Sites/data/yeast_processed_log_norm_area_50/"
...
for swissprot_id in proteins:
    ...
    PSSMs[swissprot_id] = pssm_A.to_numpy(dtype=np.float32)
...
for UA, UB, SM in zip(ppi_masks['Uniprot ID A'], ppi_masks['Uniprot ID B'], ppi_masks['Sites Masks']):
    try:
        ...
        # number according to position    
        seqA = np.arange(1, lengths[0]+1, dtype=np.float32)[np.newaxis]
        seqB = np.arange(1, lengths[1]+1, dtype=np.float32)[np.newaxis]
...
position_landscape = np.asarray(position_landscape, dtype=object)
...
for i, (protA, protB) in enumerate(zip(ppi_masks['Uniprot ID A'], ppi_masks['Uniprot ID B'])):
    ...
    landscape_df = pd.read_csv(PIPE_dir + PIPE_landscape_name + '.mat',
                               ...
                               header=None,
                               index_col=None)
    landscape = landscape_df.to_numpy().astype(np.float32)
...
if NORM and not LOAD_PARAMS:
    ...
    with open(processed_data_path + 'min_maxes.json','w') as f:
        json.dump(min_maxes,f, default=convert)


# In[32]:


ppi_masks.to_pickle(processed_data_path + 'data_processed_2021-02-09.pkl')
...
ppi_masks = pd.read_pickle(processed_data_path + 'data_processed_2021-02-09.pkl')
...
for UA, UB, site_mask, PSSM_mask, position_landscape, PIPE_landscape,  PIPE_landscape_SW in zip(test_df['Uniprot ID A'], 
    ...
    np.save(f'{processed_data_path}test/masks/{UA}_{UB}.npy', np.expand_dims(site_mask, axis=0).astype(np.float32))
    np.save(f'{processed_data_path}test/features/{UA}_{UB}.npy', features.astype(np.float32))
...
for UA, UB, site_mask, PSSM_mask, position_landscape, PIPE_landscape,  PIPE_landscape_SW in zip(val_df['Uniprot ID A'], 
    ...
    features = np.stack([PSSM_mask, position_landscape, PIPE_landscape,  PIPE_landscape_SW])
    np.save(f'{processed_data_path}val/masks/{UA}_{UB}.npy', np.expand_dims(site_mask, axis=0).astype(np.float32))
    np.save(f'{processed_data_path}val/features/{UA}_{UB}.npy', features.astype(np.float32))
...
for UA, UB, site_mask, PSSM_mask, position_landscape, PIPE_landscape,  PIPE_landscape_SW in zip(train_df['Uniprot ID A'], 
    ...
    features = np.stack([PSSM_mask, position_landscape, PIPE_landscape,  PIPE_landscape_SW])
    np.save(f'{processed_data_path}train/masks/{UA}_{UB}.npy', np.expand_dims(site_mask, axis=0).astype(np.float32))
    np.save(f'{processed_data_path}train/features/{UA}_{UB}.npy', features.astype(np.float32))
